...
import com.fudax.core.support.ExecuteListener;
import com.fudax.logging.webdriver.HTMLLogger;
import com.fudax.tools.wingui.Win32GuiByVbs;
import com.fudax.utils.LoggerUtils;
...
public class ChromeDriverController implements DriverController {
	...
	public void startServer(String runClassName, Long startTime) throws Exception {
		...
		System.out.println("==============webdriver server on " + System.getenv("USERNAME") + "@" + System.getenv("COMPUTERNAME")
				+ " successfully started at: " + service.getUrl().toString() + "==============");
	}
	...
	/**
	 * Description: quitWebDriver, close webdriver instance and clear
	 * allsessions.</BR>
	 * 内容描述：销毁WebDriver实例。
	 */
	@Override
	...
	private DesiredCapabilities getCapabilities() throws Exception {
		...
		options.addArguments("--lang=zh-CN", "allow-outdated-plugins", "--start-maximized");

		if (doptions.HEADLESS) {
	...
	private void chromeDriverEnviromentSetter() {
		...
		} else if (!System.getProperty("os.arch").equalsIgnoreCase("x86") && new File(_installed_p3 + CHROMEEXE).exists()) {
